...
class cycle_count:
    ...
    def get_cycle_count(self, layer, weights_height, input_height, input_channel, T, C=1, c_flag=False):
        ...
        if layer == 'Binary_Conv2D':
            ...
            txt += '    * Load weights         : {0:9.2f} %\n'.format(100*load_cycle_weights/cycle)
            txt += '    * Load bias            : {0:9.2f} %\n'.format(100*load_cycle_bias/cycle)
            txt += '    * Load input           : {0:9.2f} %\n'.format(100*load_cycle_input/cycle)
            txt += '    * Run PE               : {0:9.2f} %\n'.format(100*PE_cycle/cycle)
            txt += '    * Run BNA              : {0:9.2f} %'.format(100*BNA_cycle/cycle)
            print(txt)
            f.write('\n')
            f.write(txt)
            f.close()
            ...
            txt += ' TOTAL for this layer      : {0:9.0f} cycles\n'.format(int(cycle))
            txt += '    * Load input           : {0:9.2f} %\n'.format(100*load_cycle_input/cycle)
            txt += '    * Run MaxPool          : {0:9.2f} %'.format(100*Run_maxpool/cycle)
            print(txt)
            f.write('\n')
            f.write(txt)
            f.close()
            ...
            txt +='    * Run PE               : {0:9.2f} %\n'.format(100*PE_cycle/cycle)
            if c_flag:
                txt +='    * Comparison           : {0:9.2f} %'.format(100*comp_cycle/cycle)
            else:
                txt +='    * Run BNA           : {0:9.2f} %'.format(100*BNA_cycle/cycle)
            print(txt)
            f.write('\n')
        ...
        if c_flag:
            ...
            latency = self.pf.get_performace(total, self.total_PE)
            f = open(self.logfile_name, 'a')
            txt = '\n'
            txt +=' TOTAL                     : {0:9.0f} cycles\n'.format(int(total))
            txt +='    * Load weights         : {0:9.2f} %\n'.format(100*self.total_weights/total)
            txt +='    * Load bias            : {0:9.2f} %\n'.format(100*self.total_bias/total)
            txt +='    * Load input           : {0:9.2f} %\n'.format(100*self.total_input/total)
            txt +='    * Run PE               : {0:9.2f} %\n'.format(100*self.total_PE/total)
