...
class energy:
    ...
    def get_energy(self, layer_type, weights_height, input_height, input_channel, T, datasize_output, C=1, c_flag=False, Total_cycle=0, PE_cycle=0):
        ...
        if layer_type == 'Binary_Conv2D':
            ...
            energy_datamove += weights_height*config.ENERGY_DM_LOAD_CONTROL
            ## Input buffer (read)
    ...
    def get_leakage(self, total_cycle, PE_cycle):
        ...
        leakage_energy_other = leakpower_or+leakpower_bna+leakpower_comparison+leakpower_buffer_iw+leakpower_buffer_bias+leakpower_load_control
        leakage_energy_pe = leakpower_popcount+leakpower_xor

        self.leakage = leakage_energy_other*other_cycle*(config.CLOCK_PERIOD_OTHERS/1000)+leakage_energy_pe*PE_cycle*(config.CLOCK_PERIOD_PE/1000)
